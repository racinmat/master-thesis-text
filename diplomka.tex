...
probability distribution of depth pixel-wise. Li et al. reconstruct
depth as a soft weighted sum inference by 
\[
...
State of the Art results on single-view reconstruction. 

todo: víc rozepsat, taky asi víc rozepsat konvoluci a maxpooling a
dilated konvoluci?

A tímhle multicore zazipuju složku tar cf - without\_db | bzip2 >
closed\_virtualscapes.tar.gz
...
\section{\label{sec:Datasets-proposal}Datasets proposal}
...
Both datasets are publicly available and can be downloaded from \url{http://ptak.felk.cvut.cz/public_datasets/GTA_V/dataset-website/}.
...
queried for value of any node in the graph given specific input. todo:
dopsat běh na GPU, tensorboard a další věci
...
to the visualization pipeline \cite{real-time-rendering}. This pixel-wise
transformation per image is everything we need to create the dataset
feasible for training the depth prediction. 
...
\begin{figure}
\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-35--576}\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-35--972}

\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-36--349}\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-36--728}\caption{\label{fig:extracted-RGB-images}extracted RGB images from 4 cameras}

\end{figure}

\begin{figure}
\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-35--576-depth}\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-35--972-depth}

\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-36--349-depth}\includegraphics[width=7cm]{obrazky/2018-05-08--14-15-36--728-depth}\caption{\label{fig:extracted-depth-images}extracted depth images from 4 cameras}
\end{figure}
...
\begin{figure}
\begin{centering}
\includegraphics[width=15cm]{obrazky/pcl-full}
\par\end{centering}
\begin{centering}
\includegraphics[width=15cm]{obrazky/pcl-sub}
\par\end{centering}
...
\begin{figure}
\begin{centering}
\includegraphics[width=14cm]{obrazky/occupied-pcl-thicc}
\par\end{centering}
...
\begin{figure}
\begin{centering}
\includegraphics[width=14cm]{obrazky/occupied-frustum-with-camera}
\par\end{centering}
...
todo: popsat můj setup, dodat obrázky a experimenty, video na prezentaci
s rgb, depth, a mapou?, zvýšit u voxelmpay velikost bodů. popsat gta
pro ai gym a popsat rozdíly oproti mému přístupu
...
Multiple datasets are being used for depth estimation.{\small{}\tableofcontents{}% vkládá automaticky generovaný obsah dokumentu
} Most of them are indoor, since indoor we are able to measure depth
more precisely and we generally deal with smaller distances. In my
case, I trained for estimating outdoor depth. The synthetic dataset
retrieved from GTA V has big advantage in dense and high precision
depth labels seen nowhere in real datasets. In the outdoor depth estimation,
I don't care about very distant points, so the depth estimation is
thresholded 
